...
<head>
    ...
    <style>
        .profile-form-container { background: #fff; padding: 25px; border-radius: 12px; border: 1px solid #e2e8f0; margin-top: 20px; }
        .form-group { margin-bottom: 15px; text-align: left; }
        .form-group label { display: block; margin-bottom: 8px; font-weight: 600; color: #4a5568; font-size: 14px; }
        .form-group input { width: 100%; padding: 12px; border: 1px solid #cbd5e0; border-radius: 8px; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        .form-group input:focus { border-color: #0A4D8C; outline: none; box-shadow: 0 0 0 3px rgba(10, 77, 140, 0.1); }
        .form-group input:disabled { background-color: #edf2f7; cursor: not-allowed; color: #a0aec0; }
        .btn-update { background-color: #0A4D8C; color: white; padding: 12px 24px; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; width: 100%; font-family: 'Poppins', sans-serif; transition: 0.3s; }
        .btn-update:hover { background-color: #083b6b; transform: translateY(-2px); }
        .status-active { background-color: #d1fae5; color: #065f46; padding: 4px 12px; border-radius: 20px; font-weight: 600; font-size: 12px; }
        .status-inactive { background-color: #fee2e2; color: #991b1b; padding: 4px 12px; border-radius: 20px; font-weight: 600; font-size: 12px; }
    </style>
</head>
                ...
                <div class="profile-form-container">
                    ...
                    <form action="/update-profile" method="POST">
                        <div class="form-group">
                            <label for="namaLengkap">Nama Lengkap (Tidak dapat diubah)</label>
                            <input type="text" id="namaLengkap" value="<?= htmlspecialchars($nama) ?>" disabled>
                        </div>
                        <div class="form-group">
                            <label for="emailAkun">Email (Tidak dapat diubah)</label>
                            <input type="email" id="emailAkun" value="<?= htmlspecialchars($email) ?>" disabled>
                        </div>
                        <div class="form-group">
                            <label for="umur">Umur (Tahun)</label>
                            <input type="number" id="umur" name="umur" value="<?= htmlspecialchars($umur) ?>" placeholder="Masukkan umur kamu">
                        </div>
                        <div class="form-group">
                            <label for="noWa">Nomor WhatsApp</label>
                            <input type="tel" id="noWa" name="no_wa" value="<?= htmlspecialchars($no_wa) ?>" placeholder="Contoh: 081234567890">
                        </div>
